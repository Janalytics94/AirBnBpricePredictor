...
def text(source, target):
    ...
    for df_name in df_names:
        with open(os.path.join(source, df_name, df_name + ".jsonl"), "r") as stream:
            ...
            lemmas_name = [list(text[index]['lemmas_name']) for index in range(len(text))] 
            common_words_name = [list(text[index]['common_words_name']) for index in range(len(text))] 
            lemmas_description = [list(text[index]['lemmas_description']) for index in range(len(text))] 
            common_words_description = [list(text[index]['common_words_description']) for index in range(len(text))] 
            lemmas_transit = [list(text[index]['lemmas_transit']) for index in range(len(text))] 
            common_words_transit = [list(text[index]['common_words_transit']) for index in range(len(text))] 
            lemmas_house_rules = [list(text[index]['lemmas_house_rules']) for index in range(len(text))] 
            common_words_house_rules = [list(text[index]['common_words_house_rules']) for index in range(len(text))] 
            common_words_neighborhood_overview = [list(text[index]['common_words_neighborhood_overview']) for index in range(len(text))] 
            lemmas_neighborhood_overview = [list(text[index]['lemmas_neighborhood_overview']) for index in range(len(text))] 
            # languages = [text[index]['language'] for index in range(len(text))] 
